...
use tokio::process::Child; // ใช้ tokio process เพื่อควบคุมการทำงานแบบ Async

// 1. สร้างตัวแปร Global เพื่อเก็บ Process ของ FFmpeg ที่กำลังทำงานอยู่
lazy_static! {
    static ref FFMPEG_PROCESS: Arc<Mutex<Option<Child>>> = Arc::new(Mutex::new(None));
}

// 🟢 แบบที่ 1: สำหรับหน้าแอป Tauri
#[tauri::command]
async fn stop_download_tauri() -> Result<String, String> {
    let mut process_opt = FFMPEG_PROCESS.lock().await;
    
    if let Some(mut child) = process_opt.take() {
        match child.kill().await { // 👈 เติม .await ตรงนี้
            Ok(_) => {
                let _ = child.wait().await; // 👈 เติม .await ตรงนี้ด้วย
                Ok("หยุดการดาวน์โหลดเรียบร้อยแล้ว".to_string())
            },
            Err(e) => Err(format!("ไม่สามารถหยุด FFmpeg ได้: {}", e)),
        }
    } else {
        Ok("ไม่มีการดาวน์โหลดที่กำลังทำงานอยู่".to_string())
    }
}

// 🔵 แบบที่ 2: สำหรับ Extension
async fn stop_download_api() -> &'static str {
    let mut process_opt = FFMPEG_PROCESS.lock().await;
    if let Some(mut child) = process_opt.take() {
        let _ = child.kill().await; // 👈 เติม .await ตรงนี้
        let _ = child.wait().await; // 👈 เติม .await ตรงนี้
        "Download stopped successfully"
    } else {
        "No active download to stop"
    }
}
// ---------------------------------------------------------
...
fn main() {
    ...
    std::thread::spawn(|| {
        ...
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            let app = Router::new()
                ...
                .route("/stop", get(stop_download_api)) // 👈 แก้ไข: ลบเครื่องหมาย ; ตรงนี้ออก เพื่อให้มันต่อกับ .layer() ด้านล่างได้ครับ
                .layer(cors);
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            stop_download_tauri // 👈 เพิ่ม: ลงทะเบียนให้หน้า UI ของแอปเรียกคำสั่งนี้ได้
            // ถ้าคุณมี tauri::command ตัวอื่นๆ (เช่น save_config, load_config) ให้เอามาคั่นด้วยลูกน้ำ (,) ต่อท้ายตรงนี้ได้เลยนะครับ
        ])
...
async fn handle_download(Json(payload): Json<DownloadPayload>) -> &'static str {
    // 🛑 [ส่วนที่เพิ่มใหม่ 1] เคลียร์ Process เก่าก่อนเริ่มงานใหม่ (ระบบ Restart)
    {
        let mut process_opt = FFMPEG_PROCESS.lock().await;
        if let Some(mut old_child) = process_opt.take() {
            write_log("⚠️ มีการสั่งดาวน์โหลดใหม่ บังคับหยุดงานเก่าเพื่อคืนทรัพยากร...");
            let _ = old_child.kill().await; 
            let _ = old_child.wait().await; 
            write_log("🛑 ยกเลิกงานเก่าสำเร็จ เริ่มงานใหม่ได้!");
        }
    }
    ...
    // 🛡️ ชุดคำสั่งป้องกันการหลุด (สำคัญมากสำหรับโหลดจาก URL โดยตรง)
    let reconnect_args = ["-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5"];

    // 🛡️ 1. ใส่ข้อมูลพรางตัวและลิงก์ Video
    ffmpeg_cmd.args(&reconnect_args); // สั่งให้ต่ออายุถ้าหลุด
    ffmpeg_cmd.arg("-user_agent").arg(&user_agent);
    ffmpeg_cmd.arg("-headers").arg(&headers);
    ffmpeg_cmd.arg("-i").arg(&payload.video_url);

    // 🛡️ 2. เช็คไฟล์เสียง
    let has_audio = !payload.audio_url.trim().is_empty();
    if has_audio {
        ffmpeg_cmd.args(&reconnect_args); // สั่งให้ต่ออายุถ้าหลุด
        ffmpeg_cmd.arg("-user_agent").arg(&user_agent);
        ffmpeg_cmd.arg("-headers").arg(&headers);
        ffmpeg_cmd.arg("-i").arg(&payload.audio_url);
        write_log("🔊 ตรวจพบไฟล์เสียง (Audio Stream) ทำการรวมภาพและเสียง...");
    } else {
        write_log("⚠️ ไม่พบไฟล์เสียงแยกต่างหาก ดาวน์โหลดเฉพาะวิดีโอ...");
    }

    // 3. ใส่คำสั่งตาม Profile
    match config.ffmpeg_profile.as_str() {
        "mac" => {
            if has_audio {
                ffmpeg_cmd.args([
                    "-map", "0:v:0", // เอาเฉพาะสตรีมวิดีโอจากลิงก์แรก
                    "-map", "1:a:0", // เอาเฉพาะสตรีมเสียงจากลิงก์ที่ 2
                    "-c:v", "libx264", 
                    "-preset", "fast", 
                    "-c:a", "aac",   // ⚠️ บังคับแปลงเสียงใหม่เพื่อรีเซ็ต Timestamps ให้ตรงกับภาพ
                    "-b:a", "192k",
                    "-pix_fmt", "yuv420p", 
                    "-movflags", "+faststart",
                    "-shortest"      // ⚠️ ตัดจบไฟล์ทันทีเมื่อภาพหรือเสียงอันใดอันหนึ่งจบ ป้องกันอาการค้าง
                ]);
            } else {
                ffmpeg_cmd.args([
                    "-c:v", "libx264", 
        ...
        "windows" => {
            if has_audio {
                ffmpeg_cmd.args(["-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-c:a", "copy", "-shortest"]);
            } else {
    ...
    match output {
        // 📥 [ส่วนที่เพิ่มใหม่ 2] เก็บ Process ใหม่ลงไปในกล่อง เพื่อให้สั่งหยุดในอนาคตได้
        Ok(child) => {
            let mut process_opt = FFMPEG_PROCESS.lock().await;
            *process_opt = Some(child);
            "Downloading and merging in background..."
        },
        Err(e) => {
